...
export async function GET(_: NextRequest, { params }: Params) {
    ...
    const comments = await spotlightManager.getCommentsBySpotlightId(id)
    return SuccessItemsResponse(comments)
}
...
export async function POST(req: NextRequest, { params }: Params) {
    ...
    if (!body || !body.content || typeof body.content !== 'string') return ApiErrorResponse(10008)

    const comment = await spotlightManager.createComment(spotlightId, `${user.id}`, body.content)
